...
void CTK_EXPORT qListToSTLVector(const QStringList& list, std::vector<char*>& vector);

///
/// \ingroup Core
/// Convert a QStringList to a Vector of string
void CTK_EXPORT qListToSTLVector(const QStringList& list, std::vector<std::string>& vector);

///
/// \ingroup Core
/// Convert a Vector of string to QStringList
void CTK_EXPORT stlVectorToQList(const std::vector<std::string>& vector, QStringList& list);
...
QStringList CTK_EXPORT nameFilterToExtensions(const QString& nameFilter);

///
/// \ingroup Core
/// Convert a nameFilter to a list of file extensions:
/// "Images (*.png *.jpg *.tiff)", "Text (*.txt)" -> "*.png", "*.jpg", "*.tiff", "*.txt"
QStringList CTK_EXPORT nameFiltersToExtensions(const QStringList& nameFilters);

///
/// \ingroup Core
/// Convert a wildcar extension filter ("*.jpg") into a regular expression string
/// "*.jpg" -> ".*\\.jpg?$"
QString CTK_EXPORT extensionToRegExp(const QString& extension);
...
/// "*.jpg", "*.txt" -> "(.*\\.jpg?$|.*\\.txt?$)"
QRegExp CTK_EXPORT nameFiltersToRegExp(const QStringList& nameFilters);
...
int CTK_EXPORT significantDecimals(double value, int defaultDecimals = -1);
...
int CTK_EXPORT orderOfMagnitude(double value);
...
double CTK_EXPORT closestPowerOfTen(double value);
...
/// \sa QDir::rmdir
bool CTK_EXPORT removeDirRecursively(const QString & dirName);
...
/// \return <code>true</code> on success, <code>false</code> otherwise.
/// \sa QFile::copy
bool CTK_EXPORT copyDirRecursively(const QString &srcPath, const QString &dstPath);

///
/// \ingroup Core
/// Convert Qt::HANDLE to string
/// \sa Qt::HANDLE
QString CTK_EXPORT qtHandleToString(Qt::HANDLE handle);
...
qint64 CTK_EXPORT msecsTo(const QDateTime& t1, const QDateTime& t2);
