...
QString CTK_EXPORT base64HTMLImageTagSrc(const QImage& image);
...
QImage CTK_EXPORT grabWidget(QWidget* widget, QRect rectangle = QRect());

///
/// \ingroup Widgets
/// Convert an KWidget encoded image into a QImage
/// The data can be base64 encoded and/or zlib compressed.
QImage CTK_EXPORT kwIconToQImage(const unsigned char *data, int width, int height, int pixelSize, unsigned int bufferLength, int options = 0);
